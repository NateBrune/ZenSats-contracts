    ...
    contract ZenjiFuzzTest is Test {
        ...
        function testFuzz_depositAll_withdrawInOrder(
            ...
            for (uint256 i = 0; i < 4; i++) {
                ...
                assertLe(
                    w, totalDeposited + 1e5, "Single actor extracted more than total vault deposits"
                );
            }
            ...
            assertLe(
                ...
                "Total withdrawals massively exceed deposits"
            );
        ...
        function testFuzz_interleavedDepositWithdraw(
            ...
            for (uint256 i = 0; i < 4; i++) {
                assertLe(
                    totalWithdrawn[i],
                    totalDeposited * 2 + 1e5,
                    "Actor extracted more than total vault deposits"
                );
        ...
        function testFuzz_finalWithdrawDrainsVault(uint256[4] calldata rawAmounts) external {
            uint256[4] memory amounts = _boundAmounts(rawAmounts);
            ...
            assertEq(
                vault.totalSupply(), 0, "Vault not empty - isFinalWithdraw failed to close position"
            );
            assertLe(
                vault.getTotalCollateral(), 1e3, "Collateral stranded after last user withdrawal"
            );
        ...
        function testFuzz_twoUser_depositOrderNeutral(
            ...
            assertLe(
                outA + outB,
                totalDeposited * 2 + 1e5,
                "Combined withdrawals massively exceed deposits"
            );
        ...
        function _boundAmounts(uint256[4] calldata raw)
            internal
            view
            returns (uint256[4] memory bounded)
        {
            for (uint256 i = 0; i < 4; i++) {
        ...
        function _shuffleActors(address[4] memory arr, uint256 seed)
            internal
            pure
