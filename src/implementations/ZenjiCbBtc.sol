// SPDX-License-Identifier: MIT
pragma solidity ^0.8.33;

import { Zenji } from "../Zenji.sol";
import { ERC20 } from "@openzeppelin/contracts/token/ERC20/ERC20.sol";
import { IERC20Metadata } from "@openzeppelin/contracts/token/ERC20/extensions/IERC20Metadata.sol";

contract ZenjiCbBtc is Zenji {
    address private constant CBBTC = 0xcbB7C0000aB88B473b1f5aFd9ef808440eed33Bf;
    address private constant USDT = 0xdAC17F958D2ee523a2206206994597C13D831ec7;

    constructor(
        address _loanManager,
        address _yieldStrategy,
        address _swapper,
        address _owner,
        address _viewHelper
    ) Zenji(CBBTC, USDT, _loanManager, _yieldStrategy, _swapper, _owner, _viewHelper) { }

    function name() public pure override(ERC20, IERC20Metadata) returns (string memory) {
        return "Zen cbBTC (USDT->IPOR)";
    }

    function symbol() public pure override(ERC20, IERC20Metadata) returns (string memory) {
        return "zencbBTC-usdtIpor";
    }

    /// @notice 1e5 = 0.001 cbBTC ≈ $100 dead capital (at $100K/BTC).
    /// Attack cost: ~$100. Dilution: ~10 bps at $100K TVL, ~1 bp at $1M TVL.
    function VIRTUAL_SHARE_OFFSET() public pure override returns (uint256) {
        return 1e5;
    }
}
